...
function renderDocumentRows(docs,houseId,ownerMode){
  ...
  const canManage=!!ownerMode&&canOperateProperties();
  return '<div class="document-list">'+docs.map(function(d){
    const protectedPersonal=!!ownerMode&&(d.restrito||
      (d.tipo==='documento'&&!canViewSensitiveTenantData()));
    const url=protectedPersonal?'':safePrivateFileUrl(d.url||d.dados);
    const name=protectedPersonal?'Arquivo protegido':d.nome;
    const detail=protectedPersonal?'Conteúdo restrito à administração e operação':
      esc(documentTypeLabel(d.tipo))+' · '+formatFileSize(d.tamanho);
    return '<div class="document-row"><div class="document-icon">'+(d.mime==='application/pdf'?'PDF':'IMG')+'</div>'+ 
      '<div class="document-main"><strong>'+esc(name)+'</strong><span>'+detail+'</span></div>'+
      (canManage?'<label class="doc-portal-toggle"><input type="checkbox" '+(d.visivelInquilino?'checked':'')+' onchange="toggleDocumentPortal(\''+houseId+'\',\''+d.id+'\',this.checked)"><span>No portal</span></label>':'')+
      (url?'<a class="btn btn-ghost btn-sm" href="'+esc(url)+'" target="_blank" rel="noopener" download>Baixar</a>':'')+
      (canManage?'<button class="icon-action danger" onclick="confirmDeleteDocument(\''+houseId+'\',\''+d.id+'\')" aria-label="Excluir">×</button>':'')+
    '</div>';
  }).join('')+'</div>';
}
